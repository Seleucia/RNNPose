...
class erd:
   def __init__(self, n_in, n_lstm, n_out, lr=0.05, batch_size=64, single_output=True, output_activation=theano.tensor.nnet.relu,cost_function='nll'):
       ...
       self.b_fc1 = shared(np.cast[dtype](np.random.uniform(-0.5,.5,size = self.n_fc2)))

       self.W_fc2 = init_weight((self.n_fc2, self.n_out),'W_fc2')
       self.b_fc2 = shared(np.cast[dtype](np.random.uniform(-0.5,.5,size = self.n_out)))
       ...
       self.b_y = shared(np.zeros(self.n_fc1, dtype=dtype))
